	...
	for(p_node=&head;p_node;p_node=p_node->p_next){
		...
		if(p_tmp){
			if(p_tmp->num>num){
				...
				insert_node->num=num;
				p_node->p_next=insert_node;
				insert_node->p_next=p_tmp;
				return 1;
			}
		}else{
			node* insert_node=(node*)malloc(sizeof(node));
...
int insert_cb(void* p_node,void* pdata){
	node* p_tmp = ((node*)p_node)->p_next;
	rdata* rd_tmp = (rdata*)pdata;
	if(p_tmp){
		if(p_tmp->num>rd_tmp->num){
			node* insert_node=(node*)malloc(sizeof(node));
			((node*)p_node->p_next)=insert_node;
			insert_node->p_next=p_tmp;
			rd_tmp->ret=1;
			return 1;
		}else{
...
int insert_node2(num){
	rdata rd = {};
	rd.num=num;
	for_each(insert_cb,&rd);
	return rd.ret;
}
